...
const validRoutes:AppRoute[]=["dashboard","payroll","pdfs","reports","history","users","settings"];
...
export function RHControlApp({user,initialRoute="dashboard"}:{user:UserProfile;initialRoute?:AppRoute}){const[route,setRoute]=useState<AppRoute>(initialRoute);useEffect(()=>{const pop=()=>setRoute(routeFromPath());window.addEventListener("popstate",pop);return()=>window.removeEventListener("popstate",pop)},[]);const page=route==="dashboard"?<DashboardPage navigate={setRoute}/>:route==="payroll"?<PayrollPage/>:route==="pdfs"?<PdfToolsPage/>:route==="reports"?<ReportsPage/>:route==="history"?<HistoryPage/>:route==="users"?<UsersPage/>:<SettingsPage/>;return <AppProvider user={user}><AppShell route={route} onNavigate={setRoute}>{page}</AppShell><Toasts/></AppProvider>}
