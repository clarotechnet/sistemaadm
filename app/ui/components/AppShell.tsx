...
export function AppShell({ route, onNavigate, children }: { route: AppRoute; onNavigate: (route: AppRoute) => void; children: React.ReactNode }) {
  ...
  return <div className={`control-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    ...
    <aside className={`control-sidebar ${mobileOpen ? "mobile-open" : ""}`}>
      <div className="control-brand"><img className="company-logo sidebar-company-logo" src="/logo-empresa-ui.png" alt="Logo da empresa"/><div><strong>RH Control</strong><small>Gestão administrativa</small></div><button className="mobile-close" onClick={() => setMobileOpen(false)}><X size={18} /></button></div>
